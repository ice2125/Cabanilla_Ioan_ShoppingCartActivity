...
namespace ShoppingCartSystem
{
    ...
    class Program
    {
        ...
        const int MAX_CART_SLOTS = 10;
        const int MAX_TOTAL_ITEMS = 10;
                ...
                if (menuChoice == "2")
                {
                    ...
                    while (inCart)
                    {
                        ...
                        else if (choice == "3")
                        {
                            ...
                            if (diff > 0) cart[updateIndex - 1].Product.ReduceStock(diff);
                            else if (diff < 0) cart[updateIndex - 1].Product.AddStock(-diff);
                            cart[updateIndex - 1].SetQuantity(newQty);
                            Console.WriteLine("Quantity updated.");
                        }
                        else if (choice == "4")
                        {
                            for (int i = 0; i < cartItemCount; i++) { cart[i].Product.AddStock(cart[i].Quantity); cart[i] = null; }
                            cartItemCount = 0;
